...
async function assignShader(div, shaderFile) {
  ...
  const nodesArray = new Int32Array(NUM_PARTICLES * 2);
  for (let i = 0; i < NUM_PARTICLES; ++i) {
    nodesArray[(2 * i) + 0] = Math.trunc(((2 * Math.random() - 1) * (1 << 31)) * (1 - PARTICLE_SIZE));
    nodesArray[(2 * i) + 1] = Math.trunc(((2 * Math.random() - 1) * (1 << 31)) * (1 - PARTICLE_SIZE));
  }
